...
def detect(model, postprocessors, data_loader, base_ds, device, args, logger=None):
    ...
    for img_id, predict in output_per_image.items():
        ...
        img_path = os.path.join(args.coco_path, "test2017/images", img_filename)  # 你的图像目录
        # 读取和显示图像
        ...
        polys = predict[2]
        points = predict[3]
        for cls, score, poly, pts in zip(categories, scores, polys, points):
            if cls - 1 not in color_table:
                # 如果类别索引无效，跳过绘制
                continue
            poly = poly.reshape(-1, 1, 2).astype(int)
            img = cv2.polylines(img, [poly], True, color_table[cls - 1], thickness=2)
            # 在图像上显示类别和置信度（以百分数形式）
            class_name = class_names[cls - 1]["name"]
            # 将置信度转换为百分数形式
            score_percent = f"{score * 100:.2f}%"
            text = f"{class_name}: {score_percent}"
            text_size = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)[0]
            text_x = int(poly[0][0][0])
            text_y = int(poly[0][0][1] - text_size[1] - 2)
            cv2.putText(img, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_table[cls - 1], 1)
